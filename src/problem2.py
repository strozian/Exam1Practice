...
def test_problem2a():
    ...
    rectangle = rg.Rectangle(rg.Point(100, 170), rg.Point(200, 120))
...
def problem2a(circle, rectangle, window):
    ...
    point2 = rg.Point(x2, y1)
    point = rg.Point(x1, y2)
...
def problem2b(rect, n, delta, win):
    ...
    # ------------------------------------------------------------------
    for k in range(n):
        topx = abs(rect._upper_left_corner.x) - (delta)*k
        topy = abs(rect._upper_left_corner.y) - (delta)*k
        botx = abs(rect._lower_right_corner.x) + delta*k
        boty = abs(rect._lower_right_corner.y) + delta*k
        tpoint = rg.Point(topx, topy)
        bpoint = rg.Point(botx, boty)
        rect2 = rg.Rectangle(tpoint, bpoint)
        rect2.attach_to(win)
